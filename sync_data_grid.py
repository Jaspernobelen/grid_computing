...
outputgrid_dir = "gsiftp://gridftp.grid.sara.nl:2811/pnfs/grid.sara.nl/data/projects.nl/modulations/TapeAndDisk/Data/tar/"
...
#            sys.exit("There are jobs running at the moment. Please return when there are no jobs running!")
    ...
    parser = argparse.ArgumentParser(
        description='Welcome to the Copier! I will copy raw and processed data files to dCache.')
    parser.add_argument('-i', '--raw', type=int, help="Copying all the raw data")
    parser.add_argument('-p', '--process', type=int,  help="Copying all the processed data")
...
# coding convention (python standard library - pep8) flake8 = syntax checking -- 2 to 3 python script | git repository
    if (rawOn == 0):
        ...
        handles = ".bin"
        do_it_all2(handles, localfolder, gridfolder, gridfolder_all,
                   srmin, json_raw)  # niet zo'n goede functienaam
        logging.debug(' ')
        logging.debug(' ')
    if (processedOn == 0):
...
def exclude_latest_folder(localfolder):
    ...
    # Luckily, everything is saved in terms of dates, so we can easily find the latest folder (when sorted, the first element!)
...
def tarchecker(gridfolder_all):
    ...
    # This creates our output. We will analyze the strings that come out of the uberftp -ls command.
    o,  e = outputftp.communicate()
    # Splitting every line
    lines = o.split('\n')
    file_name, folder_name = [], []
    global which_files
    # Which_Files is the list in which all tar files are stored. This will be used later (hence Global)
    which_files = []
    for line in lines:
        ls_fields = line.split()
        # Our output is like this: ['/pnfs/', 'projects.nl/', 'modulations/', 'TapeAndDisk/', '<folder>', 'mx_X_YYYYMMDD_HHMM.tar']
        if len(ls_fields) <= 1:
            # Not all lines consist of information we need. We need at least the tar file, so anything <1 is wrong.
            continue
        path_fields = ls_fields[-1].split('/')
        # We restrict ourselves to 'mx_' folders.
        if path_fields[-1].startswith('mx_'):
...
def ana_cal_patcher(gridfolder, localfolder):
    ...
    if bad_cal != 0:
        for every in bad_cal:
            ...
            logging.debug('Fixed bad file ' + str(every) +
                          '. Removed old file, made a new .tar file and re-synchronized!')
    if bad_ana != 0:
...
def ana_cal_checker(gridfolder):
    ...
    ftpan = subprocess.Popen(['uberftp', '-ls', ana_output],
                             stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ftpcal = subprocess.Popen(['uberftp', '-ls', cal_output],
                              stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def ana_cal_checksum(localfolder, gridfolder, srmfolder):
    ...
    for cal_file in new_cal_files:
        if checked_cal != 0 and cal_file in checked_cal[:]:
            ...
            check_loc = subprocess.Popen(
                ['adler32', loc_cal], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            ...
            if checksum_cal_grid == checksum_cal_loc:
                ...
                caldatalog.append({"Name": cal_file, "Adler32 Checksum - LOCAL": checksum_cal_loc,
                                   "Adler32 Checksum - GRID": checksum_cal_grid, "Discrepancy": 0})
            else:
                logging.debug('File ' + str(cal_file) +
                              ' doesn\'t have the same checksum. Watch out!!')
                caldatalog.append({"Name": cal_file, "Adler32 Checksum - LOCAL": checksum_cal_loc,
                                   "Adler32 Checksum - GRID": checksum_cal_grid, "Discrepancy": 1})
    ...
    for ana_file in new_ana_files:
        if checked_ana != 0 and ana_file in checked_ana[:]:
            ...
            check_loc = subprocess.Popen(
                ['adler32', loc_ana], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            ...
            if checksum_ana_grid == checksum_ana_loc:
                ...
                anadatalog.append({"Name": ana_file, "Adler32 Checksum - LOCAL": checksum_ana_loc,
                                   "Adler32 Checksum - GRID": checksum_ana_grid, "Discrepancy": 0})
            else:
                logging.debug('File ' + str(cal_file) +
                              ' doesn\'t have the same checksum. Watch out!!')
                anadatalog.append({"Name": ana_file, "Adler32 Checksum - LOCAL": checksum_ana_loc,
                                   "Adler32 Checksum - GRID": checksum_ana_grid, "Discrepancy": 1})
...
def localchecker(localfolder):
    ...
    for all_folders in rundir:
        if all_folders.endswith('.tar'):
            # So we make a list of every folder starting with 'mx_'. If they end with '.tar', we append 'tar_fnames', else we append 'loc_foldernames'. Both shall be used.
            tar_fnames.append(all_folders)
        else:
            loc_foldernames.append(all_folders)
...
#            cmd = 'tar -C ' + folder + ' -cvf ' + badlocal + ' .'
            ...
            logging.debug('Fixed bad file ' + str(bad) +
                          '. Removed old file, made a new .tar file and re-synchronized!')
...
#    print(mismatch_tar)
    for every in mismatch_folders:  # functie maken hiervoor
        if every in most_recent:
            logging.debug('I have stumbled upon the most recent folder: ' +
                          str(every) + ' I won\'t make a tar file of this one!!')
        else:
            if os.path.exists(localfolder+every+'.tar'):
                ...
                logging.debug('Creating tar file ' + str(tarfile) +
                              ' from folder ' + str(every) + '.')
...
def syncer(localfolder, gridfolder):
    ...
    if len(mismatch_tar) != 0:
        for every in mismatch_tar:  # First copy missing tar files on the GRID
            ...
            subprocess.call(['rm', local_location])
    if len(mismatch_folders) != 0:
        for every in mismatch_folders:
            print('Synchronizing folder: ' + str(every))
            if every in most_recent:
                logging.debug('I have stumbled (again) upon the most recent folder: ' +
                              str(every) + 'I won\'t synchronize this one!!')
            else:
                tarfile = localfolder+every+'.tar'
                gridfolder_location = gridfolder + every + '.tar'
                every_folder = localfolder+every+'/'
                logging.debug('Copying tar file ' + str(tarfile) +
                              ' from folder ' + str(every) + '.')
            # Thomas: next line is commented to prevent copying
...
#    read_json(json_fname)
    ...
    for every_tarfile in new_which_files:
        if already_checked != 0 and every_tarfile in already_checked[:]:
            ...
            check_loc = subprocess.Popen(['adler32', local_tar],
                                         stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            b, e = check_loc.communicate()
            checksum_local = b.split('\n')[0]
            logging.debug('-'*60)
            logging.debug('Checksum for LOCAL file ' + str(every_tarfile) +
                          ' (adler32): ' + str(checksum_local))
            ...
            o = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE,
                                 stderr=subprocess.PIPE)  # regex
            a, e = o.communicate()
            # wrapper voor return code
            checksum_grid = a.split('\n')[0]
            logging.debug('Checksum for GRID file ' + str(every_tarfile) +
                          ' (adler32): ' + str(checksum_grid))
            if checksum_local == checksum_grid:
                logging.debug('File ' + str(every_tarfile) + ' has been transferred succesfully!')
                data_good.append({"Name": every_tarfile, "Adler32 Checksum - LOCAL": checksum_local,
                                  "Adler32 Checksum - GRID": checksum_grid, "Discrepancy": 0})
            else:
                logging.debug('File ' + str(every_tarfile) +
                              ' doesn\'t have the same checksum. Watch out!!')
                data_good.append({"Name": every_tarfile, "Adler32 Checksum - LOCAL": checksum_local,
                                  "Adler32 Checksum - GRID": checksum_grid, "Discrepancy": 1})
    if len(data_good) != 0:
...
#    print(type(o))
    ...
    file_name, folder_name = [], []
    global which_files
    # Which_Files is the list in which all files are stored. This will be used later (hence Global)
    which_files = []
    for line in lines:
        ls_fields = line.split()
        # Our output is like this: ['A', 'B', 'C', 'D', 'folder', 'file']
        if len(ls_fields) <= 1:
            # Not all lines consist of information we need. We need at least the file + folder, so anything <1 is wrong.
            continue
        path_fields = ls_fields[-1].split('/')
        # We restrict ourselves to 'mx_' folders; analysis and calibration are synchronized later.
        if path_fields[-2].startswith('mx_'):
            file_name = path_fields[-1]
            which_files.append(file_name)
    return which_files


# This is for the folder structure that our processor works with.
def grid_analyzer_deeper(localfolder, gridfolder):
    """ We store processed files in the following way:
    ...
    logging.debug('~'*60)
    logging.debug(
        'COPY_ALL::GRID_ANALYZER_DEEPER --> I WILL MAKE A LIST OF ALL THE PROCESSED/CALIBRATIONFILES ON THE GRID')
    logging.debug('~'*60)
    ...
    for a_folder in list_of_folders:
        ...
        outputftp = subprocess.Popen(['uberftp', '-ls', new_statement],
                                     stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        o,  e = outputftp.communicate()
        lines = o.split('\n')
        file_name, folder_name = [], []
...
def calibrationfolder_sync(localfolder, gridfolder):
    ...
    for a_folder in missing_folder:
        if most_recent in a_folder:
            logging.debug('I found a missing folder, ' + str(most_recent) +
                          ' but it is the most recent one. Skipping . . . ')
        else:
    ...
    for all_folders in grid_folders:
        missing_files = list(
            set(all_files_cal_local_list[all_folders])-set(all_files_cal_grid_list[all_folders]))
...
def local_analyzer(localfolder):
    list_of_folders = which_files.keys()
    # This command is an analyzer for stored files. It's will get every file starting with mx_!
    dirs = glob.glob(localfolder+'/mx_*')
    dirs = sorted(dirs, reverse=True)
    rundir = []
    for entry in dirs:
        ...
        rundir.append(run)
    global not_copied
    # I created a list of the set(LOCAL)-set(GRID). This means, if there's new local files, it won't be == 0.
    not_copied = list(set(rundir)-set(list_of_folders))
    # If you define set(GRID)-set(LOCAL), you can find out if the local storage misses anything.


# Handles will be '.root' or '.bin' dependent on raw/processed files.
def file_analyzer(handles, localfolder, gridfolder):
    """            file_analyzer:
    ...
    """
    all_handles = "*"+handles
    # The list is divided as follows: 'folder' : file1,file2,file3, 'folder2' : file1 ...
    list_of_folders = which_files.keys()
    global in_folder
    in_folder = []
    for all_files in list_of_folders:
        # This is a list comprehension: in every folder, we check for files ending with .root or .bin!
        grid_rootfiles = [x for x in which_files[all_files] if x.endswith(handles)]
...
# add a flag/checksum
        ...
        if len(missing_file) != 0:
            ...
            logging.debug("Missing the following file: " +
                          str(missing_file) + " in folder " + str(all_files))
...
def file_copier(handles, localfolder, gridfolder):
    ...
    # Again the set(A)-set(B) 'trick' - This time, if the set isn't 0, we miss some files...
    if len(folders) != 0:
        #    print('Mismatch found in folder: ' + str(all_files) + '!')
        #        print('Missing file(s): ' + str(missing_file))
        for a in folders:
            if a in most_recent:
                logging.debug('Hey! I found the most recent folder, ' + str(a) +
                              ' - I am skipping this one! (May still be processing/filling as we speak)')
            else:
                location_grid = gridfolder + a + '/'
                location_nikhef = 'file://'+localfolder + a + '/'
                # When a file is missing, the folder was either being processed, or the file transfering was interrupted.
                subprocess.call(['uberftp', '-rm', '-r', location_grid])
                # This is why I chose to remove the folder and re-copying it; in case the other files are wrong as well.
                logging.debug('Removed folder: ' + a + '/')
                subprocess.call(['uberftp', '-mkdir', location_grid])
                # This command then copies the folder back into place
                subprocess.call(['globus-url-copy', '-r', location_nikhef, location_grid])
...
def copy_ana_cal_files(localfolder, gridfolder):
    ...
    the_missing_cal = []
    # This is a list comprehension: in every folder, we check for files ending with .root or .bin!
    grid_ana = [x for x in which_otherfiles['analysis'] if x.startswith('ANA')]
...
def copy_all_missing_folders(localfolder, gridfolder):
    ...
    for tocopy in not_copied:
        if tocopy in most_recent:
            logging.debug('Hey! I found the most recent folder, ' + str(tocopy) +
                          ' - I am skipping this one! (May still be processing/filling as we speak)')
        else:
            logging.debug('Copying folder: ' + tocopy)
            string = 'file://'+localfolder+tocopy+'/'
            subprocess.call(['uberftp', '-mkdir', gridfolder+tocopy+'/'])
            # Straightforward. This one copies the folders that were missing.
            subprocess.call(['globus-url-copy', '-r', string, gridfolder+tocopy+'/'])
